...
A robust, full-stack task management application featuring ownership-based navigation, Kanban boards, and real-time UI updates.

## 1. Overview
TaskFlow is a productivity tool built to manage professional projects and team assignments. 
...
- **Custom UI Library**: Built using Tailwind v4 to ensure a lightweight footprint without the overhead of a massive external UI framework.
...
- **Frontend Bonuses**: Drag-and-drop, Dark mode, and Real-time updates (WebSockets) were left out to focus on core CRUD reliability and state persistence.
- **Backend Bonuses**: The statistics endpoint and integration tests were omitted. Instead, the focus was placed on strict schema validation, secure password hashing (Bcrypt cost 12), and robust Docker orchestration.
...
### Projects
- `GET /projects`: List all projects owned by or assigned to the user.
...
### Tasks
- `GET /projects/:id/tasks`: List tasks for a project (with status/assignee filters).
